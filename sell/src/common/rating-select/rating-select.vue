<template>
  <div class="rating-select">
    <div class="rating-type border-1px">
      <span @click="select(2)" class="block positive" :class="{'active': selectType === 2}">{{desc.all}}<span class="count">{{ratins.length}}</span></span>
    ...
    <div @click="toggleContent" class="switch" :class="{'on': onlyContent}">
      <i class="icon-font icon-check_circle"></i>
...
export default {
  ...
  computed: {
    positives () {
      return this.ratins.filter((rating) => {
